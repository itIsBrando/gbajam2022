...
direction_t dir_to_x(int x1, int x2) {
    if(x1 > x2)
...
direction_t dir_to_y(int y1, int y2) {
    if(y1 > y2)
        return DIRECTION_UP;
    else if(y1 < y2)
...
inline void bg_move_by(BG_REGULAR *bg, const int dx, const int dy)
{
    bg_move(bg,
        bg->bgx + dx,
        bg->bgy + dy
...
void bg_rect(const BG_REGULAR *bg, uint x, uint y, const uint w, const uint h, void *data)
{
    ...
    {
        const uint hw = w >> 1;
        const bool _dx = x & 1, _dw = w & 1;
        const bool isOddEnd = (_dw && !_dx) || (_dx && !_dw);
        u8 *tiles = (u8*)data;

        x >>= 1;
        ptr = map_mem[bg->map_base] + (y << (bg->width_bits - 1)) + x;
        
        for(j = 0; j < h; j++) {
            if(_dx) {
                *ptr = (*ptr & 0xff) | (*tiles << 8);
                ptr++;
                tiles++;
            }
            
            if(hw != 0) {
                for (i = 0; i < hw - (isOddEnd ? 1 : 0); i++) {
                    // damn pointer alignment require this jank
                    const u16 t = (tiles[1] << 8) | tiles[0];
                    *ptr++ = t;
                    tiles += 2;
                }
            }
            ...
            if(isOddEnd) {
                *ptr = (*ptr & 0xff00) | *tiles;
                *ptr++;
                tiles++;
            }


            ptr += ((1 << bg->width_bits) - w - _dx - isOddEnd) >> 1;
        }
    } else {
...
void bg_fill(const BG_REGULAR *bg, uint x, uint y, uint w, uint h, u16 tile)
{
    ...
    if(bg->is_affine)
    {
        tile &= 0xff; // affine tiles are only 8-bit
        tile = (tile << 8) | tile; // but we are doing 16-bit writes hehehe
        x >>= 1;
        y >>= 1;
