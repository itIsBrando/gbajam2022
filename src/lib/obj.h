...
/**
 * @param x initial x position (0-255)
 * @param y initial y position (0-512)
 * @param tile initial tile (0-1024)
 * @note must be freed with `spr_free`
 * @returns pointer to your new sprite.
 */
 ...
 */
void spr_copy_all();
void spr_copy(obj_t *, const u16 index);
...
/**
 * @param x absolute x position (0-511)
 * @param y absolute y position (0-255)
 */
void spr_move(obj_t *, const u16, const u16);
...
/**
 * Sets the priority of a sprite. Higher priorities are drawn first.
 ...
 * @note This function has no effect in 8BPP mode.
 * @param pal (0x0-0xF)
 */
 ...
 * @note this is NOT safe on affine sprites.
 * @see spr_flip_mask
 */
void spr_flip(obj_t *obj, const spr_flip_mask mask);
